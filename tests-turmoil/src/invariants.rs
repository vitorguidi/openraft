...
pub fn check_state_invariants(snapshots: &[(NodeId, FullNodeSnapshot)]) -> InvariantCheckResult {
    ...
    // Find all nodes that are or have been leaders in their current term
    let mut leaders_by_term: HashMap<u64, NodeId> = HashMap::new();
    for (node_id, s) in snapshots {
        let leader_id = s.raft.vote.leader_id();
        if leader_id.node_id == *node_id {
            let term = leader_id.term;
            match leaders_by_term.get(&term) {
                Some(_) => {
                    // Already have a leader for this term, election safety check will catch this if they differ
                }
                None => {
                    leaders_by_term.insert(term, *node_id);
                }
            }
        }
    }

    // Collect all committed log IDs from all nodes
    let mut committed_log_ids: HashMap<u64, LogId> = HashMap::new();
    for (_, s) in snapshots {
        if let Some(committed) = s.raft.committed {
            let index = committed.index();
            for idx in 0..=index {
                if let Some(log_id) = s.raft.log_ids.get(idx) {
                    match committed_log_ids.get(&idx) {
                        Some(existing) if existing.committed_leader_id().term > log_id.committed_leader_id().term => {
                            committed_log_ids.insert(idx, log_id);
                        }
                        None => {
                            committed_log_ids.insert(idx, log_id);
                        }
    ...
    // Check all leaders against all entries committed in earlier terms
    for (leader_term, leader_node_id) in &leaders_by_term {
        let s = &snapshots.iter().find(|(id, _)| *id == *leader_node_id).unwrap().1;

        for (idx, committed_id) in &committed_log_ids {
            // Only check entries committed in terms EARLIER than the leader's term
            if committed_id.committed_leader_id().term >= *leader_term {
                continue;
            }

            let leader_log_id = s.raft.log_ids.get(*idx);
            let leader_purged = s.raft.log_ids.purged().map(|id| id.index()).unwrap_or(0);

            if let Some(lid) = leader_log_id {
                if lid.committed_leader_id().term != committed_id.committed_leader_id().term {
                    println!(
                        "LEADER COMPLETENESS: Leader {} (term {}) has index {} with term {}, but it was committed with term {}",
                        leader_node_id, leader_term, idx, lid.committed_leader_id().term, committed_id.committed_leader_id().term
                    );
            ...
            } else if *idx > leader_purged {
                println!(
                    "LEADER COMPLETENESS: Leader {} (term {}) missing committed entry at index {}",
                    leader_node_id, leader_term, idx
                );
                violations.push(InvariantViolation::LeaderMissingCommitted {
                    term: *leader_term,
                    leader: *leader_node_id,
